...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_image'])) {
    ...
    if (isset($_FILES['new_image_file']) && $_FILES['new_image_file']['error'] == UPLOAD_ERR_OK) {
        ...
        $file_type = strtolower(pathinfo($file_name, PATHINFO_EXTENSION)); // $file_type holds extension here
        ...
        // Validate file type - MODIFIED
        $allowed_extensions = ['webp']; // ONLY WEBP
        if ($uploadOk == 1 && !in_array($file_type, $allowed_extensions)) {
            $message = '<div class="alert alert-danger">Sorry, only WEBP files are allowed.</div>';
            $uploadOk = 0;
        }

        // Validate file size - MODIFIED
        $max_file_size = 1048576; // 1 MB in bytes
        if ($uploadOk == 1 && $_FILES['new_image_file']['size'] > $max_file_size) {
            $message = '<div class="alert alert-danger">Sorry, your file is too large (max 1MB).</div>';
            $uploadOk = 0;
        }

        if ($uploadOk == 1) { // Only proceed if validation passed
            // Move the new file
